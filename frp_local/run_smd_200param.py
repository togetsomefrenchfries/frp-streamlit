...
# -*- coding: utf-8 -*-
"""
750参数超参数优化实验 - Comments=1且BU列为SMD的数据
750-Parameter Hyperparameter Optimization Experiment - Comments=1 and BU=SMD Data Only
...
3. 5折交叉验证
4. 每5个配置保存一次结果
5. 只使用Comments=1且BU列为SMD的数据进行训练和验证
"""
...
class ValidDataLoader:
    """有效数据加载器 - 只加载Comments=1且BU列为SMD的数据"""
    ...
    def load_valid_data(self):
        ...
        # 2. BU列（第73列）为SMD
        bu_col = raw_data.iloc[:, 72]
        smd_mask = bu_col.astype(str).str.upper() == 'SMD'
        
        # 组合条件
        valid_mask = comments_mask & smd_mask
        valid_data = raw_data[valid_mask].copy()
        
        print(f"🎯 数据筛选结果:")
        print(f"  原始数据: {len(raw_data)} 行")
        print(f"  Comments=1: {comments_mask.sum()} 行")
        print(f"  BU列=SMD: {smd_mask.sum()} 行")
        print(f"  Comments=1且BU=SMD: {len(valid_data)} 行")
...
class ValidDataExperimentRunner:
    ...
    def save_final_results(self, best_results):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"数据筛选条件: Comments=1 且 BU列=SMD\n")
...
def main():
    ...
    print("筛选条件: Comments=1 且 BU列=SMD")
    print("=" * 50)
