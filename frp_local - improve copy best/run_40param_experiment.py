...
# -*- coding: utf-8 -*-
"""
FRP钢筋耐久性预测 - 140参数优化实验 (预定义特征版)

特点：
1. 三种机器学习模型：RandomForest, XGBoost, LightGBM
2. RandomForest 100个配置，XGBoost和LightGBM各20个配置，总计140个配置
3. 5折交叉验证
...
class ValidDataLoader:
    ...
    def prepare_features_target(self, data, generate_plots=False):
        ...
        # 提取权重 - 确保权重与过滤后的数据对应
        all_sample_weights = self._extract_sample_weights(data_clean)
        # 使用combined_clean的索引来获取对应的权重
        sample_weights = all_sample_weights[combined_clean.index]
...
def get_140_parameter_configs():
    """获取140个参数配置：RF 100个，其他两个各20个"""
    
    configs = []
    
    # RandomForest 参数配置 (100个)
    rf_configs = []
    
    # 基础配置矩阵
    n_estimators_list = [50, 80, 100, 120, 150, 180, 200, 250, 300, 350, 400, 450, 500]
    max_depth_list = [3, 4, 5, 6, 7, 8, 10, 12, 15, 20, None]
    min_samples_split_list = [2, 3, 5, 8, 10]
    min_samples_leaf_list = [1, 2, 4, 6, 8]
    max_features_list = ['sqrt', 'log2', None, 0.3, 0.5, 0.7]
    
    # 生成100个RandomForest配置
    import itertools
    param_combinations = list(itertools.product(
        n_estimators_list[:8],  # 取前8个n_estimators
        max_depth_list[:6],     # 取前6个max_depth
        min_samples_split_list[:3],  # 取前3个min_samples_split
    ))
    
    # 选择前100个组合
    for i, (n_est, max_d, min_split) in enumerate(param_combinations[:100]):
        config = {
            'n_estimators': n_est,
            'max_depth': max_d,
    ...
    if XGBOOST_AVAILABLE:
        xgb_configs = [
            ...
            {'n_estimators': 80, 'max_depth': 4, 'learning_rate': 0.2, 'random_state': 42},
            {'n_estimators': 350, 'max_depth': 5, 'learning_rate': 0.06, 'random_state': 42},
            {'n_estimators': 280, 'max_depth': 6, 'learning_rate': 0.04, 'random_state': 42},
            {'n_estimators': 160, 'max_depth': 7, 'learning_rate': 0.13, 'random_state': 42},
            {'n_estimators': 380, 'max_depth': 3, 'learning_rate': 0.02, 'random_state': 42},
            {'n_estimators': 90, 'max_depth': 8, 'learning_rate': 0.18, 'random_state': 42},
            {'n_estimators': 220, 'max_depth': 4, 'learning_rate': 0.11, 'random_state': 42}
        ]
    ...
    if LIGHTGBM_AVAILABLE:
        lgb_configs = [
            ...
            {'n_estimators': 80, 'max_depth': 4, 'learning_rate': 0.18, 'num_leaves': 15, 'random_state': 42},
            {'n_estimators': 400, 'max_depth': 5, 'learning_rate': 0.03, 'num_leaves': 31, 'random_state': 42},
            {'n_estimators': 160, 'max_depth': 7, 'learning_rate': 0.13, 'num_leaves': 127, 'random_state': 42},
            {'n_estimators': 320, 'max_depth': 3, 'learning_rate': 0.09, 'num_leaves': 7, 'random_state': 42},
            {'n_estimators': 90, 'max_depth': 8, 'learning_rate': 0.16, 'num_leaves': 255, 'random_state': 42},
            {'n_estimators': 270, 'max_depth': 4, 'learning_rate': 0.06, 'num_leaves': 15, 'random_state': 42},
            {'n_estimators': 380, 'max_depth': 6, 'learning_rate': 0.02, 'num_leaves': 63, 'random_state': 42},
            {'n_estimators': 140, 'max_depth': 5, 'learning_rate': 0.14, 'num_leaves': 31, 'random_state': 42}
        ]
...
def save_results(results, experiment_id):
    ...
    csv_path = result_dir / f"140param_exp_{experiment_id}.csv"
    df.to_csv(csv_path, index=False, encoding='utf-8')
    
    # 保存详细结果为JSON
    json_path = result_dir / f"140param_exp_{experiment_id}_detailed.json"
...
def main():
    ...
    print("📋 RandomForest: 100个配置")
    print("📋 XGBoost: 20个配置") 
    print("📋 LightGBM: 20个配置")
    ...
    # 3. 获取参数配置
    print("\n⚙️  准备参数配置...")
    configs = get_140_parameter_configs()
