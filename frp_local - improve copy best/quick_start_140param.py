...
使用方法：
1. 确保数据文件存在
2. 运行此脚本： python quick_start_140param.py
3. 查看实验结果

这个脚本会：
- 自动检查环境和数据
- 运行140参数优化实验（RF 100个，XGBoost 20个，LightGBM 20个）
- 支持样品权重训练
- 支持average_area作为diameter备用数据源
- 生成详细的结果报告
"""
...
def show_results():
    ...
    # 查找最新的140参数实验结果
    csv_files = list(experiments_dir.glob("140param_exp_*.csv"))
    if not csv_files:
        print("❌ 未找到140参数实验结果文件")
        # 也查找旧的40参数结果
...
def main():
    """主函数"""
    print("🎯 FRP 140参数实验 - 快速启动")
    print("=" * 50)
    print("📋 RandomForest: 100个配置")
    print("📋 XGBoost: 20个配置")
    print("📋 LightGBM: 20个配置")
    print("📋 支持样品权重训练")
    print("📋 支持average_area作为diameter备用数据源")
    print("=" * 50)
