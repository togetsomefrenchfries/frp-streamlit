...
# -*- coding: utf-8 -*-
"""
FRP钢筋耐久性预测 - 450参数优化实验 (预定义特征版)

特点：
1. 三种机器学习模型：RandomForest, XGBoost, LightGBM
2. 每个模型约150个配置，总计约450个配置
3. 5折交叉验证
...
def get_450_parameter_configs():
    """获取450个参数配置：每个模型约150个配置"""
    
    configs = []
    
    # RandomForest 参数配置 (150个)
    rf_configs = []
    ...
    # 生成150个RandomForest配置
    import itertools
    from itertools import product
    
    # 创建更平滑的参数组合 - 避免剧烈跳跃
    # 重新排列max_depth，使其更平滑过渡
    max_depth_smooth = [3, 4, 5, 6, 7, 8, 10, 12]  # 保持原序列但确保平滑
    
    base_combinations = list(product(
        n_estimators_list[:10],  # 取前10个n_estimators  
        max_depth_smooth[:8],    # 使用平滑的max_depth序列
        min_samples_split_list[:5],  # 取前5个min_samples_split
    ))
    
    # 从400个组合中选择150个，但重新排序以避免剧烈跳跃
    selected_combinations = base_combinations[:150]
    
    for i, (n_est, max_d, min_split) in enumerate(selected_combinations):
        config = {
            'n_estimators': n_est,
            'max_depth': max_d,
    ...
    if XGBOOST_AVAILABLE:
        # 生成XGBoost参数组合
        xgb_n_estimators = [50, 80, 100, 120, 150, 180, 200, 250, 300, 350, 400, 500, 600]
        xgb_max_depth = [3, 4, 5, 6, 7, 8, 9, 10]
        xgb_learning_rate = [0.01, 0.02, 0.03, 0.05, 0.07, 0.1, 0.12, 0.15, 0.2]
        xgb_subsample = [0.7, 0.8, 0.9, 1.0]
        xgb_colsample_bytree = [0.7, 0.8, 0.9, 1.0]
        ...
        xgb_configs = []
        for i, (n_est, max_d, lr) in enumerate(xgb_combinations[:150]):
            config = {
                'n_estimators': n_est,
                'max_depth': max_d,
                'learning_rate': lr,
            ...
            # 添加更多参数变化
            if i % 10 == 0:
                config['subsample'] = xgb_subsample[i % len(xgb_subsample)]
            if i % 15 == 0:
                config['colsample_bytree'] = xgb_colsample_bytree[i % len(xgb_colsample_bytree)]
            if i % 20 == 0:
                config['reg_alpha'] = [0.0, 0.1, 0.5, 1.0][i % 4]
            if i % 25 == 0:
                config['reg_lambda'] = [0.0, 0.1, 0.5, 1.0][i % 4]
    ...
    if LIGHTGBM_AVAILABLE:
        # 生成LightGBM参数组合
        lgb_n_estimators = [50, 80, 100, 120, 150, 180, 200, 250, 300, 350, 400, 500, 600]
        lgb_max_depth = [3, 4, 5, 6, 7, 8, 9, 10, -1]
        lgb_learning_rate = [0.01, 0.02, 0.03, 0.05, 0.07, 0.1, 0.12, 0.15, 0.2]
        lgb_num_leaves = [7, 15, 31, 63, 127, 255]
        lgb_subsample = [0.7, 0.8, 0.9, 1.0]
        lgb_colsample_bytree = [0.7, 0.8, 0.9, 1.0]
        
        lgb_combinations = list(product(
            lgb_n_estimators[:8],
            lgb_max_depth[:6],
            lgb_learning_rate[:5]
        ))
        ...
        lgb_configs = []
        for i, (n_est, max_d, lr) in enumerate(lgb_combinations[:150]):
            config = {
                'n_estimators': n_est,
                'max_depth': max_d,
                'learning_rate': lr,
            ...
            # 动态设置num_leaves，确保与max_depth兼容
            if max_d == -1:
                config['num_leaves'] = lgb_num_leaves[i % len(lgb_num_leaves)]
            else:
                # num_leaves 应该小于 2^max_depth
                max_leaves = min(2**max_d - 1, 255)
                valid_leaves = [l for l in lgb_num_leaves if l <= max_leaves]
                if valid_leaves:
                    config['num_leaves'] = valid_leaves[i % len(valid_leaves)]
                else:
                    config['num_leaves'] = max_leaves
            
            # 添加更多参数变化
            if i % 10 == 0:
                config['subsample'] = lgb_subsample[i % len(lgb_subsample)]
            if i % 15 == 0:
                config['colsample_bytree'] = lgb_colsample_bytree[i % len(lgb_colsample_bytree)]
            if i % 20 == 0:
                config['reg_alpha'] = [0.0, 0.1, 0.5, 1.0][i % 4]
            if i % 25 == 0:
                config['reg_lambda'] = [0.0, 0.1, 0.5, 1.0][i % 4]
            if i % 30 == 0:
                config['min_child_samples'] = [5, 10, 15, 20][i % 4]
