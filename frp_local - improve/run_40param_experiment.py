...
# -*- coding: utf-8 -*-
"""
FRP钢筋耐久性预测 - CatBoost 200参数优化实验 (预定义特征版)

特点：
1. 专门优化CatBoost机器学习模型
2. 200个CatBoost超参数配置
3. 5折交叉验证
4. 基于database 4.xlsx真实结构的特征提取
5. 使用13个预定义工程特征进行训练
6. 权重训练已禁用（使用均匀权重）
7. 支持average_area作为diameter备用数据源
8. 特征工程：对diameter进行开根号变换，提高线性关系
9. 数据筛选：只包含Glass和Basalt纤维类型的数据
10. 全面的CatBoost参数搜索策略
11. 增强的错误处理和进度跟踪
"""
...
try:
    ...
    print("⚠️  CatBoost未安装，将跳过CatBoost实验")
...
class ValidDataLoader:
    ...
    def clean_data(self, df):
        ...
        for pos in numeric_positions:
            if pos < len(df_clean.columns):
                for idx in range(len(df_clean)):  # 使用range而不是df_clean.index
                    value = df_clean.iloc[idx, pos]
                            ...
                            try:
                                ...
                                if numbers:
                                    ...
                                    if not np.isnan(new_value):
                                        df_clean.iloc[idx, pos] = new_value
    ...
    def analyze_data_quality_and_generate_plots(self, data):
        ...
        for i, (feature_name, config) in enumerate(feature_position_mapping.items(), 1):
            ...
            # 创建图形
            fig, axes = plt.subplots(2, len(config['positions']), figsize=(5*len(config['positions']), 10))
            if len(config['positions']) == 1:
            ...
            for j, (pos, col_name) in enumerate(zip(config['positions'], config['names'])):
                if pos < len(data.columns):
                    ...
                    # 第一行：数据质量分布饼图
                    ax1 = axes[0, j] if len(config['positions']) > 1 else axes[0, 0]
                    self._plot_data_quality_pie(ax1, quality_stats, col_name)
                    
                    # 第二行：有效数据分布图
                    ax2 = axes[1, j] if len(config['positions']) > 1 else axes[1, 0]
    ...
    def prepare_features_target(self, data, generate_plots=False):
        """
        ...
        Args:
            data: 输入数据DataFrame
            generate_plots: 是否生成特征分布图，默认False
        """
        print("🔧 开始基于真实结构的特征提取...")
        print("=" * 80)
        
        # 根据参数决定是否进行数据质量分析并生成分布图
        if generate_plots:
            print("📊 开始数据质量分析和特征分布图生成...")
            print("=" * 80)
        ...
        feature_position_mapping = {
            ...
            'initial_tensile_strength': {
                'primary_positions': [34, 37, 40],  # 初始拉伸强度相关列
                'secondary_positions': [35, 38, 41],  # 备选位置
                'type': 'numerical',
            ...
            #     'type': 'numerical',
            #     'description': '拉伸强度保持率 (0-1) - 已注释掉避免数据泄漏'
            # },
            'surface_treatment': {
        ...
        # 1. 检查Target_parameter列（AE列，第31列，索引30）
        target_param_valid = pd.Series(True, index=data_clean.index)
        if len(data_clean.columns) > 30:  # AE列 (第31列)
            target_param_col = data_clean.iloc[:, 30]  # AE列
            target_param_valid = target_param_col.str.lower().eq('tensile')
            tensile_count = target_param_valid.sum()
            print(f"  Target_parameter检查: {tensile_count}行为'Tensile'或'tensile'")
        
        # 2. 检查retention1列（第101列，索引100）
        retention1_valid = pd.Series(True, index=data_clean.index)
        if len(data_clean.columns) > 100:  # retention1列
            retention1_col = data_clean.iloc[:, 100]
            retention1_numeric = pd.to_numeric(retention1_col, errors='coerce')
            retention1_valid = retention1_numeric.notna()
            retention1_count = retention1_valid.sum()
            print(f"  retention1检查: {retention1_count}行有数值")
        
        # 3. 检查condition_time是否有值（不允许缺失）
        condition_time_valid = pd.Series(True, index=data_clean.index)
        condition_time_col = None
        for feat_name, feat_data in feature_data.items():
            if 'condition_time' in feat_name:
                condition_time_valid = feat_data.notna()
                condition_time_count = condition_time_valid.sum()
                print(f"  condition_time检查: {condition_time_count}行有值（缺失值将被排除）")
                break
        
        # 4. 检查纤维类型是否为Glass或Basalt
        fiber_type_valid = pd.Series(True, index=data_clean.index)
        if len(data_clean.columns) > 8:  # Fiber_type列（第9列，索引8）
            fiber_type_col = data_clean.iloc[:, 8].astype(str).str.lower()
            # 只保留包含'glass'或'basalt'的行
            glass_mask = fiber_type_col.str.contains('glass', na=False)
            basalt_mask = fiber_type_col.str.contains('basalt', na=False)
            fiber_type_valid = glass_mask | basalt_mask
            
            glass_count = glass_mask.sum()
            basalt_count = basalt_mask.sum()
            total_valid = fiber_type_valid.sum()
            print(f"  纤维类型检查: Glass={glass_count}行, Basalt={basalt_count}行, 总有效={total_valid}行")
        
        # 4. 计算每行的非缺失特征数量（不包括目标变量）
        feature_count_per_row = X_raw.count(axis=1)
        target_valid = target_data.notna()
        
        # 合并所有筛选条件
        min_features_required = 7
        all_conditions = (
            (feature_count_per_row >= min_features_required) &  # 至少7个特征有值
            target_valid &                                        # 目标变量有值
            target_param_valid &                                  # Target_parameter为Tensile
            retention1_valid &                                    # retention1有数值
            condition_time_valid &                                # condition_time有值
            fiber_type_valid                                      # 纤维类型为Glass或Basalt
        )
        
        combined_clean = combined[all_conditions]
        
        print(f"\n📈 严格数据质量检查结果:")
        print(f"  原始数据: {len(data_clean)} 行")
        print(f"  特征矩阵: {X_raw.shape}")
        print(f"  至少{min_features_required}个特征有值: {(feature_count_per_row >= min_features_required).sum()} 行")
        print(f"  Target_parameter为Tensile: {target_param_valid.sum()} 行") 
        print(f"  retention1有数值: {retention1_valid.sum()} 行")
        print(f"  condition_time有值: {condition_time_valid.sum()} 行")
        print(f"  纤维类型为Glass/Basalt: {fiber_type_valid.sum()} 行")
        print(f"  所有条件都满足: {len(combined_clean)} 行")
        ...
        # 对剩余的缺失值进行专业填充 - 基于材料科学领域知识
        print(f"🔧 按照材料科学规则处理缺失值...")
        X_clean_filled = self._fill_missing_values_by_domain_knowledge(X_clean, combined_clean)
        X_clean = X_clean_filled
        ...
        # 提取权重 - 确保权重与过滤后的数据对应
        # 重置索引以避免索引越界问题
        combined_clean_reset = combined_clean.reset_index(drop=True)
        # 创建对应长度的权重数组
        sample_weights = np.ones(len(combined_clean))
    ...
    def _extract_sample_weights(self, data):
        """提取样品权重 - 当前设置为禁用权重训练"""
        print("🔍 跳过样品权重提取，使用均匀权重...")
        print("  ⚠️  权重训练已禁用，所有样本使用相同权重")
        
        # 返回全为1的权重数组（等同于无权重训练）
        sample_weights = np.ones(len(data))
        print(f"  权重统计: 全部为1.000 (均匀权重)")
        return sample_weights
    
    def _fill_missing_values_by_domain_knowledge(self, X_clean, combined_clean):
        """
        基于材料科学领域知识的缺失值填充策略
        """
        ...
        for col in feature_names:
            ...
            print(f"  处理特征 {col} 的 {missing_count} 个缺失值...")
            
            # 根据特征名称判断处理策略
            if 'Glass_or_Basalt' in col or 'Vinyl_ester_or_Epoxy' in col:
                # Fibre Type / Resin Type - 分类变量
                # 缺失值设为0 (表示基准类别：Glass/Epoxy)
                X_filled[col] = X_clean[col].fillna(0)
                print(f"    分类特征缺失值填充为0 (基准类别)")
                ...
                # Fibre Content - 用固定值78.2%填充
                fixed_val = 78.2
                X_filled[col] = X_clean[col].fillna(fixed_val)
                print(f"    纤维含量缺失值用固定值填充: {fixed_val}%")
                
            elif 'initial_tensile_strength' in col:
                # Initial Tensile Strength - 用中位数填充
                median_val = X_clean[col].median()
                X_filled[col] = X_clean[col].fillna(median_val)
                print(f"    初始拉伸强度缺失值用中位数填充: {median_val:.3f}MPa")
                
            elif 'Tensile_strength_retention' in col:
                # Initial Tensile Strength - 用中位数填充
                median_val = X_clean[col].median()
                X_filled[col] = X_clean[col].fillna(median_val)
                print(f"    拉伸强度缺失值用中位数填充: {median_val:.3f}MPa")
                
            elif 'load_value' in col:
                # Sustained Load Level - 无加载记为0
                X_filled[col] = X_clean[col].fillna(0)
                print(f"    载荷水平缺失值填充为0 (无加载)")
                
            elif 'condition_time' in col:
                # Exposure Time - 用中位数填充
                median_val = X_clean[col].median()
                X_filled[col] = X_clean[col].fillna(median_val)
                print(f"    暴露时间缺失值用中位数填充: {median_val:.3f}天")
                
            elif 'Temperature' in col:
                # Exposure Temperature - 缺失值填充为25°C (室温)
                X_filled[col] = X_clean[col].fillna(25.0)
                print(f"    温度缺失值填充为25°C (室温)")
                
            elif 'Chloride_ion' in col or 'concrete' in col:
                # Solution/Medium 二元变量 - 缺失默认为0
                X_filled[col] = X_clean[col].fillna(0)
                print(f"    环境条件缺失值填充为0 (无该条件)")
                
            elif 'pH' in col:
                # pH处理 - 需要根据concrete条件判断
                ph_filled = X_clean[col].copy()
                ...
                if concrete_col is not None:
                    # 如果在混凝土中且pH缺失，填充为13 (碱性)
                    concrete_mask = X_clean[concrete_col] == 1
                    ph_missing_mask = X_clean[col].isna()
                    concrete_ph_missing = concrete_mask & ph_missing_mask
                    
                    ph_filled[concrete_ph_missing] = 13.0
                    print(f"    混凝土环境下pH缺失值填充为13 (碱性): {concrete_ph_missing.sum()}个")
                
                # 其他pH缺失用中位数填充
                remaining_missing = ph_filled.isna()
                if remaining_missing.any():
                    median_ph = ph_filled.median()
                    ph_filled[remaining_missing] = median_ph
                    print(f"    其他pH缺失值用中位数填充: {median_ph:.2f}")
                
                X_filled[col] = ph_filled
                
            elif 'surface_treatment' in col:
                # Surface Treatment - 缺失默认为0 (无处理)
                X_filled[col] = X_clean[col].fillna(0)
                print(f"    表面处理缺失值填充为0 (无处理)")
                
            elif 'glass_transition_temperature' in col:
                # Glass Transition Temperature - 用中位数填充
                median_val = X_clean[col].median()
                X_filled[col] = X_clean[col].fillna(median_val)
                ...
                if X_clean[col].dtype in ['int64', 'float64']:
                    median_val = X_clean[col].median()
                    X_filled[col] = X_clean[col].fillna(median_val)
                    print(f"    数值特征缺失值用中位数填充: {median_val:.3f}")
                else:
                    # 分类特征用众数填充
                    mode_val = X_clean[col].mode().iloc[0] if len(X_clean[col].mode()) > 0 else 0
                    X_filled[col] = X_clean[col].fillna(mode_val)
                    print(f"    分类特征缺失值用众数填充: {mode_val}")
        
        print(f"✅ 缺失值填充完成，最终数据形状: {X_filled.shape}")
        
        # 🧮 特征工程：对diameter进行开根号变换
        print("🧮 应用特征工程变换...")
        for col in X_filled.columns:
            if 'diameter' in col:
                original_values = X_filled[col].copy()
                # 确保所有值都是正数（diameter应该是正值）
                positive_values = np.maximum(original_values, 0.001)  # 避免0或负值
                # 应用开根号变换
                sqrt_values = np.sqrt(positive_values)
                X_filled[col] = sqrt_values
                
                # 统计变换效果
                original_mean = original_values.mean()
                sqrt_mean = sqrt_values.mean()
                print(f"    diameter开根号变换: {original_mean:.3f}mm → sqrt({original_mean:.3f}) = {sqrt_mean:.3f}")
        ...
        if feature_name == 'Chloride_ion':
            ...
            if pos < len(data.columns):
                ...
                if feature_name == 'Glass_or_Basalt':
                    encoded = pd.Series(0, index=data.index)  # 默认为Basalt (0)
                    glass_mask = col_data.str.contains('glass', na=False)
                    basalt_mask = col_data.str.contains('basalt', na=False)
                    
                    # 只处理明确的Glass和Basalt值，其他类型设为NaN
                    valid_mask = glass_mask | basalt_mask
                    encoded = pd.Series(float('nan'), index=data.index)
                    encoded.loc[glass_mask] = 1  # Glass=1
                    encoded.loc[basalt_mask] = 0  # Basalt=0
...
def get_450_parameter_configs():
    ...
    if CATBOOST_AVAILABLE:
        ...
        # CatBoost 参数范围
        cb_iterations = [100, 150, 200, 250, 300, 400, 500, 600, 800, 1000]
        cb_depth = [3, 4, 5, 6, 7, 8, 9, 10]
        cb_learning_rate = [0.01, 0.03, 0.05, 0.07, 0.1, 0.12, 0.15, 0.2, 0.25, 0.3]
        cb_l2_leaf_reg = [1, 3, 5, 7, 9, 11, 15, 20]
        cb_bagging_temperature = [0.0, 0.1, 0.5, 1.0, 2.0]
        cb_random_strength = [0.0, 0.1, 0.5, 1.0, 2.0]
        cb_border_count = [32, 64, 128, 254]
        cb_bootstrap_type = ['Bayesian', 'Bernoulli', 'MVS', 'Poisson']
        
        # 生成基础参数组合
        base_combinations = list(product(
            cb_iterations[:8],        # 8个iterations值
            cb_depth[:6],            # 6个depth值  
            cb_learning_rate[:5]     # 5个learning_rate值
        ))
        
        # 从240个组合中选择200个
        selected_combinations = base_combinations[:200]
        
        cb_configs = []
        for i, (iterations, depth, lr) in enumerate(selected_combinations):
            config = {
            ...
            # 添加额外参数的平滑变化
            if i % 8 == 0:  # 每8个配置改变l2_leaf_reg
                config['l2_leaf_reg'] = cb_l2_leaf_reg[i % len(cb_l2_leaf_reg)]
            
            if i % 10 == 0:  # 每10个配置改变bagging_temperature
                config['bagging_temperature'] = cb_bagging_temperature[i % len(cb_bagging_temperature)]
            
            if i % 12 == 0:  # 每12个配置改变random_strength
                config['random_strength'] = cb_random_strength[i % len(cb_random_strength)]
            
            if i % 15 == 0:  # 每15个配置改变border_count
                config['border_count'] = cb_border_count[i % len(cb_border_count)]
            
            if i % 20 == 0:  # 每20个配置改变bootstrap_type
                config['bootstrap_type'] = cb_bootstrap_type[i % len(cb_bootstrap_type)]
                # 根据bootstrap_type调整相关参数
                if config['bootstrap_type'] == 'Bernoulli':
                    config['subsample'] = [0.6, 0.7, 0.8, 0.9][i % 4]
                elif config['bootstrap_type'] == 'Poisson':
                    config['subsample'] = [0.8, 0.9, 1.0][i % 3]
            
            if i % 25 == 0:  # 每25个配置添加其他参数而不是reg_lambda
                config['max_ctr_complexity'] = [1, 2, 3, 4][i % 4]
                
            if i % 30 == 0:  # 每30个配置改变采样相关参数
                config['rsm'] = [0.5, 0.7, 0.8, 0.9, 1.0][i % 5]  # random subspace method
                
            if i % 35 == 0:  # 每35个配置添加特征重要性相关参数
                config['feature_border_type'] = ['Median', 'Uniform', 'UniformAndQuantiles'][i % 3]
        ...
        print("⚠️  CatBoost不可用，无法生成配置")
    
    return configs

def train_and_evaluate_config(config_info, X, y, sample_weights=None, cv_folds=5):
    """训练和评估单个配置 - 支持样品权重"""
    ...
    try:
        ...
        if model_name == 'RandomForest':
            ...
            # CatBoost配置
            config_cb = config.copy()
            # CatBoost通常不需要额外的配置修改，但确保一些关键参数设置正确
            config_cb.update({
                'verbose': False,
                'allow_writing_files': False,
                'thread_count': -1  # 使用所有可用CPU
            })
            
            model = cb.CatBoostRegressor(**config_cb)
            X_for_training = X  # CatBoost使用原始数据
        else:
        ...
        if sample_weights is not None:
            X_train, X_test, y_train, y_test, weights_train, weights_test = train_test_split(
                X_for_training, y, sample_weights, test_size=0.2, random_state=42
            )
        else:
...
def save_results(results, experiment_id):
    ...
    csv_path = result_dir / f"450param_exp_{experiment_id}.csv"
    df.to_csv(csv_path, index=False, encoding='utf-8')
    
    # 保存详细结果为JSON
    json_path = result_dir / f"450param_exp_{experiment_id}_detailed.json"
...
def main():
    ...
    print("📋 CatBoost: 200个配置")
    print("📋 使用预定义工程特征，与app.py保持一致")
    print("📋 权重训练已禁用，使用均匀权重和average_area数据源")
    print("📋 专门优化CatBoost模型超参数")
    ...
    # 2. 准备特征和目标变量
    print("\n🔧 准备预定义特征和目标变量...")
    result = loader.prepare_features_target(data, generate_plots=False)  # 默认不生成图
    ...
    # 3. 获取参数配置
    print("\n⚙️  准备参数配置...")
    configs = get_450_parameter_configs()
    ...
    for i, config_info in enumerate(config_iterator, 1):
        ...
        result = train_and_evaluate_config(config_info, X, y, sample_weights)
